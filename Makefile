...
# --- rules ------------------------------------------------------------------
.PHONY: all stackusagetest nodetest selfhosttest linuxabitest run run-rtl8139 run-virtio-net run-hda test rtl8139test virtionettest virtioblktest virtiorngtest virtioconsoletest nvmetest floppytest parttest blockdevtest raidtest ahcitest atapitest atalba48test idedmatest virtiogputest svgatest usbstoragetest usbkbdtest ehcitest xhcitest usbbottest layouttest layoutrendertest desktoptest ipctest hdatest httpdtest jstest lxinettest claudetest waylandtest firefoxtest firefoxpagetest lanreachabletest claudebashtest firefoxonscreentest faultvaddrtest ext2racetest bcachetest check check-all clean
...
$(LXROOT)/lxcowprot: tools/lx/lxcowprot.c
	@mkdir -p $(LXROOT)
	$(CC) -static-pie -O2 -o $@ $<
	@echo "  HOSTCC  $@ (mprotect must not break copy-on-write, or forget a dirty page)"

$(LXROOT)/lxnone: tools/lx/lxnone.c
	@mkdir -p $(LXROOT)
	$(CC) -static-pie -O2 -o $@ $<
...
$(LXROOT)/lxisa: tools/lx/lxisa.c
	@mkdir -p $(LXROOT)
	$(CC) -static-pie -O0 -o $@ $<
	@echo "  HOSTCC  $@ (which vector ISAs can actually execute under us)"

$(LXROOT)/lxtime: tools/lx/lxtime.c
	@mkdir -p $(LXROOT)
	$(CC) -static-pie -O2 -o $@ $< -lpthread
	@echo "  HOSTCC  $@ (absolute deadlines: a timed wait is neither a hang nor a spin)"

$(LXROOT)/lxnbpipe: tools/lx/lxnbpipe.c
	@mkdir -p $(LXROOT)
	$(CC) -static-pie -O2 -o $@ $<
	@echo "  HOSTCC  $@ (O_NONBLOCK on a pipe: the self-pipe drain every event loop runs)"

$(LXROOT)/lxpollout: tools/lx/lxpollout.c
	@mkdir -p $(LXROOT)
	$(CC) -static-pie -O2 -o $@ $<
	@echo "  HOSTCC  $@ (POLLOUT on a FULL socket: poll promised what the write refused)"

$(LXROOT)/lxpty: tools/lx/lxpty.c
	@mkdir -p $(LXROOT)
	$(CC) -static-pie -O2 -o $@ $< -lutil
	@echo "  HOSTCC  $@ (openpty from a Linux binary, and a full pty that reports what it took)"

$(LXROOT)/lxshcow: tools/lx/lxshcow.c
	@mkdir -p $(LXROOT)
	$(CC) -static-pie -O2 -o $@ $<
	@echo "  HOSTCC  $@ (a fork must not copy-on-write a MAP_SHARED page)"

$(LXROOT)/lxcage3: tools/lx/lxcage3.c
	@mkdir -p $(LXROOT)
	$(CC) -static-pie -O2 -o $@ $<
	@echo "  HOSTCC  $@ (JSC's pointer cage, argument-for-argument from the trace)"

$(LXROOT)/lxgcage: tools/lx/lxgcage.c
	@mkdir -p $(LXROOT)
	$(CC) -static-pie -O2 -o $@ $<
	@echo "  HOSTCC  $@ (reserve 8 GiB, trim to alignment, commit pieces: JSC's pointer cage)"

$(LXROOT)/lxwait: tools/lx/lxwait.c
	@mkdir -p $(LXROOT)
	$(CC) -static-pie -O2 -o $@ $< -lpthread
	@echo "  HOSTCC  $@ (fork/wait4/WNOHANG: the subprocess lifecycle Claude Code hung on)"

$(LXROOT)/lxsig: tools/lx/lxsig.c
	@mkdir -p $(LXROOT)
	$(CC) -static-pie -O2 -o $@ $<
	@echo "  HOSTCC  $@ (rt_sigaction/rt_sigprocmask honoured, not acknowledged)"

$(LXROOT)/lxfutex: tools/lx/lxfutex.c
	@mkdir -p $(LXROOT)
	$(CC) -static-pie -O2 -o $@ $< -lpthread
...
$(LXROOT)/lxtls: tools/lx/lxtls.c
	@mkdir -p $(LXROOT)
	$(CC) -static-pie -O2 -o $@ $< -lpthread
	@echo "  HOSTCC  $@ (every thread must see its own TLS, and a forked child must keep it)"

$(LXROOT)/lxlock: tools/lx/lxlock.c
	@mkdir -p $(LXROOT)
	$(CC) -static-pie -O2 -o $@ $<
	@echo "  HOSTCC  $@ (fcntl record locks + flock(2): both existed, neither was wired)"

$(LXROOT)/lxmsg: tools/lx/lxmsg.c
	@mkdir -p $(LXROOT)
	$(CC) -static-pie -O2 -o $@ $<
	@echo "  HOSTCC  $@ (sendmsg reported ENETUNREACH -- on a Unix socket -- for six different failures)"

$(LXROOT)/lxtlsmany: tools/lx/lxtlsmany.c
	@mkdir -p $(LXROOT)
	$(CC) -static-pie -O2 -o $@ $< -lpthread
	@echo "  HOSTCC  $@ (120 threads reading %fs:0x28 -- the scale Firefox dies at)"

$(LXROOT)/lxsockopt: tools/lx/lxsockopt.c
	@mkdir -p $(LXROOT)
	$(CC) -static-pie -O2 -o $@ $<
...
$(LXROOT)/lxnread: tools/lx/lxnread.c
	@mkdir -p $(LXROOT)
	$(CC) -static-pie -O2 -o $@ $<
	@echo "  HOSTCC  $@ (FIONREAD was answered 'that descriptor is not a terminal')"

$(LXROOT)/lxrelolib.so: tools/lx/lxrelolib.c
	@mkdir -p $(LXROOT)
	$(CC) -shared -fPIC -O2 -o $@ $<
	@echo "  HOSTCC  $@ (4096 R_X86_64_RELATIVE pointers in .data.rel.ro, for ld.so to relocate)"

$(LXROOT)/lxrelo: tools/lx/lxrelo.c $(LXROOT)/lxrelolib.so
	@mkdir -p $(LXROOT)
	$(CC) -O2 -o $@ $< -L$(LXROOT) -l:lxrelolib.so -Wl,-rpath,/
	@echo "  HOSTCC  $@ (DYNAMIC on purpose: this tests ld.so's relocation arithmetic)"

$(LXROOT)/lxwrite: tools/lx/lxwrite.c
	@mkdir -p $(LXROOT)
	$(CC) -static-pie -O2 -o $@ $<
	@echo "  HOSTCC  $@ (the kernel half of Claude Code's Edit tool: openat/write/fsync/rename on ext2)"

$(LXROOT)/lxmapcmp: tools/lx/lxmapcmp.c
	@mkdir -p $(LXROOT)
	$(CC) -static-pie -O2 -o $@ $<
	@echo "  HOSTCC  $@ (a whole mapped library must equal its own file, both ways round)"

$(LXROOT)/lxpriv: tools/lx/lxpriv.c
	@mkdir -p $(LXROOT)
	$(CC) -static-pie -O2 -o $@ $<
	@echo "  HOSTCC  $@ (a MAP_PRIVATE file mapping must be private ACROSS PROCESSES)"

$(LXROOT)/lxcow: tools/lx/lxcow.c
	@mkdir -p $(LXROOT)
	$(CC) -static-pie -O2 -o $@ $< -lpthread
	@echo "  HOSTCC  $@ (concurrent COW breaks after fork must not lose writes)"

# The JavaScriptCore reproducers. Claude Code is a Bun standalone executable, so
# the runtime that crashes in it is JSC; a 230 MB TUI driven through a terminal
# is a poor reproducer for heap corruption and a thirty-line script that WALKS
# AND VERIFIES a live object graph is a good one. Staged as data, not built.
$(LXROOT)/gcstress.js: tools/lx/gcstress.js
	@mkdir -p $(LXROOT)
	@cp -f $< $@
	@echo "  STAGE   $@ (JSC heap integrity: a verified object graph across 40 collections)"

$(LXROOT)/gcstress2.js: tools/lx/gcstress2.js
	@mkdir -p $(LXROOT)
	@cp -f $< $@
	@echo "  STAGE   $@ (...the same, plus fork+exec of a real subprocess each round)"

$(LXROOT)/lxlongname: tools/lx/lxlongname.c
	@mkdir -p $(LXROOT)
	$(CC) -static-pie -O2 -o $@ $<
	@echo "  HOSTCC  $@ (a directory listing must give back the name that is there, to 255 chars)"

$(LXROOT)/lxepoll: tools/lx/lxepoll.c
	@mkdir -p $(LXROOT)
	$(CC) -static-pie -O2 -o $@ $<
	@echo "  HOSTCC  $@ (edge-triggered epoll must not lose an edge across an unobserved drain)"

$(LXROOT)/lxanon: tools/lx/lxanon.c
	@mkdir -p $(LXROOT)
	$(CC) -static-pie -O2 -o $@ $<
	@echo "  HOSTCC  $@ (the anonymous shared file every Wayland client makes)"
...
$(LXROOT)/lxcwd: tools/lx/lxcwd.c
	@mkdir -p $(LXROOT)
	$(CC) -static-pie -O2 -o $@ $<
	@echo "  HOSTCC  $@ (every way a runtime can ask whether / exists)"

# lxgl links DYNAMICALLY, against libEGL/libGLESv2, and that is deliberate:
# the whole question is whether the guest's loader can find and run the Mesa we
# staged. A static build would answer a different question. DT_NEEDED resolves
# to the staged Mesa in-guest, not to this host's glvnd. (M2349)
$(LXROOT)/lxgl: tools/lx/lxgl.c
	@mkdir -p $(LXROOT)
	$(CC) -O2 -o $@ $< -lEGL -lGLESv2
	@echo "  HOSTCC  $@ (eight links from a GL call to the host iGPU)"

$(LXROOT)/lxdrm: tools/lx/lxdrm.c
	@mkdir -p $(LXROOT)
	$(CC) -static-pie -O2 -o $@ $<
	@echo "  HOSTCC  $@ (the four questions Mesa asks a render node)"

$(LXROOT)/lxmemfd: tools/lx/lxmemfd.c
	@mkdir -p $(LXROOT)
	$(CC) -static-pie -O2 -o $@ $<
	@echo "  HOSTCC  $@ (who owns the pages behind a memfd mapping)"
...
# fail honestly.
# THE GTK3 CONTROL, WHICH WAS BUILT BY HAND AND THEREFORE NOT REBUILT (M2297).
# It links the host's real libgtk-3.so.0 -- the same one Firefox uses -- so it
# cannot be static; the closure is already in the image because Firefox needs
# it. It is a make target now because a control that silently keeps an old
# binary is worse than no control.
$(LXROOT)/lxgtk3: tools/lx/lxgtk3.c
	@mkdir -p $(LXROOT)
	@$(CC) -O2 -o $@ $< $$(pkg-config --cflags --libs gtk+-3.0) -lwayland-client
	@echo "  CC      $@ (GTK3 control, dynamic against the host's libgtk-3)"
...
bcachetest:
	@tests/run-bcache-tests.sh

ext2racetest: $(KERNEL) $(DISK)
	@tests/run-ext2-race-test.sh

faultvaddrtest: $(KERNEL) $(DISK)
	@tests/run-fault-vaddr-test.sh
...
# M2071: the shell protects quoted characters with bit 7, and SYS_linux_run
# copied those bytes into a Linux program's argv verbatim -- Claude Code's own
# crash report showed three U+FFFD where a quoted argument's spaces should be.
lxargsplittest:
	@tests/run-lxargsplit-tests.sh

# OS-DEV's own XKB keymap, compiled by the real libxkbcommon (M1984).
xkbtest:
	@tests/run-xkb-tests.sh
...
# -fstack-usage, plus objdump for the direct call graph. No QEMU, no boot.
stackusagetest:
